...
a persistence unit "${artifactId}PersistenceUnit" which uses the JakartaEE default database.
...
The configuration can be found in "${artifactId}/pom.xml":
...
artifacts ("${artifactId}-ejb.jar" and "${artifactId}-web.war") are already built. You can build

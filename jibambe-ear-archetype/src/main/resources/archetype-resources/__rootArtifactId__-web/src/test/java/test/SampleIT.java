...
public class SampleIT {
    ...
    public static Archive<?> getEarArchive() {
        ...
        EnterpriseArchive ear = ShrinkWrap.create(EnterpriseArchive.class, "${parentArtifactId}-ear.ear");

        // Current directory is the root of the "${artifactId}" project. Go up one level
        // and enter the "${parentArtifactId}-ejb" project.
        // The ejb jar is found in the "target" directory:
        File f = new File("../${parentArtifactId}-ejb/target/${parentArtifactId}-ejb.jar");
        JavaArchive ejbJar = ShrinkWrap.create(ZipImporter.class, "${parentArtifactId}-ejb.jar").importFrom(f).as(JavaArchive.class);
        ear.addAsModule(ejbJar);

        // Now grab the web archive:
        f = new File("./target/${artifactId}.war");
        if (f.exists() == false) {
            throw new RuntimeException("File " + f.getAbsolutePath() + " does not exist.");
        }
        WebArchive war = ShrinkWrap.create(ZipImporter.class, "${artifactId}.war").importFrom(f).as(WebArchive.class);
        ear.addAsModule(war);
        ...
        // ear.as(ZipExporter.class).exportTo(new File("c:${symbol_escape}${symbol_escape}temp${symbol_escape}${symbol_escape}test.ear"), true);
